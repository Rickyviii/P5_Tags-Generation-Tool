...
with open(os.path.join(ThisDir, r"files/models/nmf_LSVC_semi_supervised_model.pkl"),   "rb") as f:
...
def question():
    ...
    if request.method == 'POST':
        ...
        if model in ['LDA', 'NMF', 'STOVF']:
            ...
            #print(launching the background task)
            with ThreadPoolExecutor() as executor:
                task1 = executor.submit(functions.create_tags, input_user, tag_model, list_words, list_tags, unsup)
                list_tags_output_ = task1.result()
